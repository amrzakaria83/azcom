        ...
        <div class="card-header border-0 pt-6">
            ...
            <div class="card-toolbar">
                ...
                <div class="d-flex justify-content-end dbuttons">
                    <a href="{{route('teacher.users.edit', $data[0]['user_id'] )}}" class="btn btn-flex btn-primary h-40px fs-7 fw-bold">
                    ...
                    <tbody>
                        @foreach ($data as $item)
                            <tr class="py-5 fw-semibold  border-bottom border-gray-300 fs-6">
                                <td>
                                    <a href="{{route('teacher.students.show', $item->id)}}" class="fs-6 text-gray-800 text-hover-primary" >{{$item->name}}</a>
                                </td>
                                <td>
                                    <a href="{{route('teacher.students.show', $item->id)}}" class="fs-6 text-gray-800 text-hover-primary">{{$item->phone}}</a>
                                </td>
                            </tr>
                        @endforeach
