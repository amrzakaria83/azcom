...
console.log(data);
  // Function to calculate parent values
    // function calculateParentValues(data) {
    //     const parentValueSums = {};
    //     const selfReferencingItems = {};

    //     data.forEach(item => {

    //     if (item.parent_id !== null) {
    //         parentValueSums[item.parent_id] = (parentValueSums[item.parent_id] || 0) + item.value;
    //         item.parent_value = parentValueSums[item.parent_id];
    //         selfReferencingItems[item.id] = data.filter(otherItem => otherItem.parent_id === item.id);
    //         // console.log(selfReferencingItems[item.id]);
  ...
  // Function to recursively build the jstree nodes
    function buildTree(nodes, parent) {
        var result = [];
        for (var i = 0; i < nodes.length; i++) {
        if (nodes[i].parent_id === parent) {
            var node = {
            "id": nodes[i].id, // Assuming 'id' is the unique identifier
            "text": '<span class="text-dark">' + nodes[i].name_ar + '</span>' +
                    '-<span class="text-info">(' + nodes[i].name_en + ')</span>' +
                    '<span class="' + (nodes[i].value < 0 ? 'text-danger' : 'text-dark') + '">(' + nodes[i].value + ')</span>' +
                    '<span class="' + (nodes[i].parent_value < 0 ? 'text-danger' : 'text-dark') + '">(' + nodes[i].parent_value + ')</span>',
            "children": buildTree(nodes, nodes[i].id)
            };
            // console.log(nodes);
...
// const updatedData = calculateParentValues(data);
// console.log(updatedData);

  // Build the jstree using the updated data
