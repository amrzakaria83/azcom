...
$(document).ready(function() {
  // Fetch the data from your model
    ...
    var data =   data.sort((a, b) => a.id - b.id);
    function buildTree(nodes, parent) {
        ...
        if (nodes[i].parent_id === parent) {
            ...
            "text": '<span class="text-dark">' + nodes[i].name_ar + '</span>' +
                    ...
                    // '<span class="' + (nodes[i].parent_value < 0 ? 'text-danger' : 'text-dark') + '">اجمالى(' + nodes[i].parent_value + ')</span>'+
                    // '<span class="' + (nodes[i].targete < 0 ? 'text-warning' : 'text-success') + '">المستهدف(' + nodes[i].targete + ')</span>' +
                    // '<span class="' + (nodes[i].parent_targete < 0 ? 'text-danger' : 'text-dark') + '">اجمالى(' + nodes[i].parent_targete + ')</span>',
