...
@section('content')
    ...
    <div class="table-responsive">
        <table class="table align-middle table-rounded table-striped table-row-dashed fs-6" id="kt_datatable_tabletemp">
            ...
            <tbody class="text-gray-600 fw-bold text-center" >
                ...
                <tr data-repeater-list="kt_docs_repeater_basic">
                    ...
                    <tr data-repeater-item>
                        <td>
                            <select data-placeholder="Select an option" required class="input-text form-control form-select mb-3 mb-lg-0 text-center product-select" name="product_id" data-kt-repeater="select2" data-control="select2">
                                <option disabled selected>Select an option</option>
                        ...
                        <td><input type="number" class="form-control mb-2 mb-md-0 text-center quantity-input" required placeholder="Number" value="1" data-kt-repeater="quantityproduc" name="quantityproduc"/></td>
                        <td><input type="number" class="form-control mb-2 mb-md-0 text-center price-input" required placeholder="Number" data-kt-repeater="sellpriceproduct" name="sellpriceproduct"/></td>
                        <td>
                            <div class="input-group mb-0">
                                <span class="input-group-text"><i class="fas fa-percentage"></i></span>
                                <input type="number" class="form-control mb-2 mb-md-0 text-center percent-input" required placeholder="%" data-kt-repeater="percent" name="percent"/>
                            </div>
                        </td>
                        <td><input type="number" class="form-control mb-2 mb-md-0 text-center price-ph-input" required placeholder="Number" data-kt-repeater="sellpriceph" name="sellpriceph"/></td>
                        <td><input type="number" class="form-control mb-2 mb-md-0 text-center total-input" required placeholder="Number" value="0" data-kt-repeater="sellpricetotal" name="sellpricetotal" disabled/></td>
                        <td>
            ...
            <!-- Add button outside the tbody -->
            <div class="form-group mt-5">
                <input data-repeater-create type="button" value="Add" class="btn btn-light-primary"/>
            </div>
        </table>
    </div>
</div>
...
$(document).ready(function () {
    ...
    $('#kt_docs_repeater_basic').repeater({
        ...
        isFirstItemUndeletable: false, // Prevent the first row from being deleted
        ready: function () {
            ...
            $(document).on('change', '[data-kt-repeater="select2"]', function () {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        console.log(data.percent); // Log the total value for debugging
            ...
            $(document).on('change', '[data-kt-repeater="select2"], [data-kt-repeater="sellpriceph"], [data-kt-repeater="quantityproduc"], [data-kt-repeater="percent"]', function () {
                ...
                const itemSellPrice = currentItem.find('[data-kt-repeater="sellpriceproduct"]');

                itemSellPrice.prop('disabled', true);
            });
        }
    });
});
...
<script>
    $(document).ready(function() {
        ...
        $('#add-item').click(function(e) {
            ...
            const formData = {
                ...
                valued_time: $('#valued_time').val(),
                note: $('#note').val(),
                method_for_payment: $('#method_for_payment').val(),
                status_order: $('#status_order').val(),
                note1: $('#note1').val(),
                note2: $('#note2').val(),
                note3: $('#note3').val(),
                quantityproduc: $('#quantityproduc').val(),
                sellpriceproduct: $('#sellpriceproduct').val(),
                percent: $('#percent').val(),
                sellpriceph: $('#sellpriceph').val(),
                sellpricetotal: $('#sellpricetotal').val()
            };
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    response.tempsaleparent.forEach(item => {
                        const row = `
                            <tr>
                                ...
                                <td>
                                    <a href="javascript:;" class="btn btn-danger delete-item" data-id="${item.id}">حذف</a>
                                </td>
                            </tr>
                        `;
                    ...
                    // Clear form fields
                    $('#prod_id, #prodstockid, #unite_id, #factor_unit, #sellprice, #note').val('').trigger('change');
    ...
    function item_remove(itemId) {
        $.ajax({
            url: `/admin/temp_sale_recs/delete/${itemId}`,
            type: 'DELETE',
            success: function(response) {
                console.log(response); // Log the response for debugging
