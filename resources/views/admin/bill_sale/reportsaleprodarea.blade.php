    ...
    <div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_header_nav'}" class="page-title d-flex align-items-center flex-wrap me-3 mb-5 mb-lg-0">
        ...
        <ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
            <li class="breadcrumb-item text-muted px-2">
                <a  href="{{route('admin.cut_sales.index')}}" class="text-muted text-hover-primary">{{trans('lang.administrators')}}</a>
            </li>
        ...
        <div class="card-body p-9">
                ...
                <div class="row mb-6">
                    ...
                    <table class="table align-middle table-rounded table-striped table-row-dashed fs-6" id="kt_datatable_table">
                        ...
                        <tr>
                            <td colspan="3"><span class="text-info">{{ $product['product_name'] }}</span> - {{trans('lang.total')}} {{trans('lang.sales')}}: {{ $product['total_product_sales'] }} {{trans('lang.unit')}}</td>
...
<script>
    $("#kt_datepicker_1").flatpickr({
        defaultDate: "{{ $fromdata->format('Y-m-d') }}"
    });
    
    $("#kt_datepicker_2").flatpickr({
        defaultDate: "{{ $todata->format('Y-m-d') }}"
    });
</script>
<script>
        // Date change handler
    $('#kt_datepicker_1, #kt_datepicker_2').change(function() {
        ...
        const newUrl = `${baseUrl}?from_time=${fromDate}&to_date=${toDate}`;
