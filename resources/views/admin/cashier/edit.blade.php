    ...
    <div id="kt_app_content_container" class="app-container container-fluid">
        <div class="card mb-5 mb-xl-10">
            ...
            <div id="kt_account_settings_profile_details" class="collapse show">
                <!--begin::Form-->
                <form action="{{route('admin.cashiers.update')}}" method="POST" enctype="multipart/form-data" id="kt_account_profile_details_form" class="form">
                    @csrf
                    ...
                    <div class="row mb-6">
                        ...
                        <div class="row mb-6">
                            <label class="col-lg-2 col-form-label fw-semibold fs-6">
                                <span class="required">اسم بند الحسابات</span>
                            </label>
                            <div class="col-sm-3 d-flex align-items-center text-center">
                                <select class="form-select text-center" autofocus required aria-label="Select example" value="{{$data->acctree_id}}" id="acctree_id" name="acctree_id" >
                                    @foreach ($data1 as $asd)
                                    <option value="{{ $asd->id }}" @if($asd->id == $data->acctree_id) selected @endif>{{ $asd->name_ar }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
