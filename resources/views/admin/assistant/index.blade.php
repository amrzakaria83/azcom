...
@section('content')
    ...
    <div id="kt_app_content_container" class="app-container container-fluid">
            <h1>{{trans('lang.assistants')}}-{{trans('lang.all')}}</h1>
                ...
                <div class="card-header border-0 pt-6">
                    ...
                    <div class="card-toolbar">
                        ...
                        <div class="d-flex justify-content-end dbuttons">
                            <!-- <a href="{{route('admin.assistants.create')}}" class="btn btn-sm btn-icon btn-primary btn-active-dark me-3 p-3">
                    ...
                    <table class="table align-middle table-rounded table-striped table-row-dashed fs-6" id="kt_datatable_table">
                        ...
                        <thead class="bg-light-dark pe-3">
                            ...
                            <tr class="text-start text-dark fw-bold fs-4 text-uppercase gs-0">
                                ...
                                <th class="min-w-125px text-start">{{trans('employee.phone')}}-{{trans('employee.address')}}</th>
                                <th class="min-w-125px text-start">{{trans('lang.center')}}</th>
                            ...
                            <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h3 class="modal-title" >{{trans('lang.work_hours')}}-{{trans('lang.addnew')}}</h3>
                                            ...
                                            <div class="modal-body">
                                                <form action="{{route('admin.assistants.storeworkhour')}}" method="POST" enctype="multipart/form-data" id="kt_account_profile_details_form" class="form">
                                                    @csrf
                                                    <input type="hidden"  id="assistantid" value="" name="id_model"/>
                                                    <input type="hidden"  id="model" value="assistant" name="model_name"/>
                                                    ...
                                                    <div class="row mb-6">
                                                        <label class="col-lg-4 col-form-label required  fw-semibold fs-6">{{trans('lang.days')}}-{{trans('lang.work')}}</label>
...
<script>
    $(function () {
        ...
        var table = $('#kt_datatable_table').DataTable({
            ...
            ajax: {
                url: "{{ route('admin.assistants.index') }}",
                data: function (d) {
...
$('#kt_modal_1b').on('show.bs.modal', function (event) {
    ...
    var assistantId = button.data('assistantId');
    var assistantname = button.data('assistantname');
    var titleElement = $('#assistantname');
    var idassistant = $('#assistantid');
    titleElement.text(assistantname);
    idassistant.val(assistantId);
