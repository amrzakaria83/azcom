                ...
                <div class="card-header border-0 pt-6">
                    ...
                    <div class="card-toolbar">
                        ...
                        <div class="d-flex justify-content-end dbuttons">
                            @can('tool new')
                            <a href="{{route('admin.tools.create')}}" class="btn btn-sm btn-icon btn-primary btn-active-dark me-3 p-3">
                ...
                <div class="row mb-6">
                        ...
                        <div class="col-sm-4">
                            ...
                            <div class="col-sm-12 fv-row">
                                <select  data-placeholder="Select an option" class=" input-text form-control  form-select  mb-3 mb-lg-0 text-center" id="empreceved_id" name="empreceved_id" data-control="select2" >
                        ...
                        <div class="col-sm-4">
                            <label class="col-sm-8 fw-semibold fs-6 mb-2">{{trans('lang.type_type')}}-{{trans('lang.name')}}</label>
                            <div class="col-sm-12 fv-row">
                                <select  data-placeholder="Select an option" class=" input-text form-control  form-select  mb-3 mb-lg-0 text-center" id="type_tool_id" name="type_tool_id" data-control="select2" >
                                    <option  disabled selected>Select an option</option>
                                        @foreach (\App\Models\Type_tool::where('status' , 0)->get() as $asd)
                                            <option value="{{$asd->id}}">{{$asd->name_en}}</option>
                                            @endforeach
                                </select>
                            </div>
                    ...
                    <table class="table align-middle table-rounded table-striped table-row-dashed fs-6" id="kt_datatable_table">
                        ...
                        <thead class="bg-light-dark pe-3">
                            ...
                            <tr class="text-start text-dark fw-bold fs-4 text-uppercase gs-0">
                                ...
                                <th class="min-w-125px text-start">{{trans('lang.name')}}-{{trans('lang.employee')}}</th>
                                <th class="min-w-125px text-start">{{trans('lang.description')}}</th>
                                <th class="min-w-125px text-start">{{trans('lang.note')}}</th>
                                <th class="min-w-125px text-start">{{trans('employee.status')}}</th>
...
<script>
    $(function () {
        ...
        var table = $('#kt_datatable_table').DataTable({
            ...
            ajax: {
                ...
                data: function (d) {
                    ...
                    d.empreceved_id = $('#empreceved_id').val(),
                    d.type_tool_id = $('#type_tool_id').val(),
                    d.search = $('#search').val()
                }
            },
            columns: [
                ...
                {data: 'name', name: 'name'},
                {data: 'type_tool_id', name: 'type_tool_id'},
                {data: 'empreceved_id', name: 'empreceved_id'},
        ...
        $('#empreceved_id,#type_tool_id').change(function() {
            table.draw();
        });
