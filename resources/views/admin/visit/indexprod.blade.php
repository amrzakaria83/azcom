            ...
            <div class="card no-border">
                ...
                <div class="card-header border-0 pt-6">
                    ...
                    <div class="card-toolbar">
                        ...
                        <div class="d-flex justify-content-end dbuttons">
                            <!-- <a href="{{route('admin.visits.create')}}" class="btn btn-sm btn-icon btn-primary btn-active-dark me-3 p-3">
                ...
                <div class="card-body py-4">
                    <!-- <div class="row mb-6">
                        ...
                        <div class="col-sm-4">
                            <label class="col-sm-8 fw-semibold fs-6 mb-2">{{trans('lang.first_call')}} {{trans('lang.products')}}</label>
                        ...
                        <div class="col-sm-4">
                            <label class="col-sm-8 fw-semibold fs-6 mb-2">{{trans('lang.second_call')}} {{trans('lang.products')}}</label>
                            <div class="col-sm-8 d-flex align-items-center">
                                <select   data-placeholder="Select an option" class=" input-text form-control  form-select  mb-3 mb-lg-0" id="secondprodstep_id" name="secondprodstep_id">
                                    <option value="{{null}}" >{{trans('lang.all')}}</option>
                                    @foreach (\App\Models\Product::where('status' , 0)->get() as $item)
                                        <option value="{{$item->id}}">{{$item->name_en}}</option>
                                    @endforeach
                        ...
                        <div class="col-sm-4">
                            <label class="col-sm-8 fw-semibold fs-6 mb-2">{{trans('lang.third_call')}} {{trans('lang.products')}}</label>
                            <div class="col-sm-8 d-flex align-items-center">
                                <select   data-placeholder="Select an option" class=" input-text form-control  form-select  mb-3 mb-lg-0" id="thirdprodstep_id" name="thirdprodstep_id">
                                    <option value="{{null}}" >{{trans('lang.all')}}</option>
                                    @foreach (\App\Models\Product::where('status' , 0)->get() as $item)
                                        <option value="{{$item->id}}">{{$item->name_en}}</option>
                                    @endforeach
                    ...
                    <div class="row mb-6">
                        ...
                        <div class="col-sm-4">
                            <label class="col-sm-8 fw-semibold fs-6 mb-2"></label>
                                <div class="col-sm-8 d-flex align-items-center">
                                    <a href="{{route('admin.visits.reportprod')}}" class="btn btn-primary btn-lg " id="searchbtn" >
                                        refresh
                                    </a> 
                                </div> 
                        </div>
                    ...
                    @if(isset($countprod))
                        <h1 class="text-danger">
                            ...
                            <span>{{ $countprod[0][13] }}</span>
                            <span class="text-dark">{{ trans('lang.end_to') }}</span>
                            <span>{{ $countprod[0][14] }}</span>
                            <span class="text-dark">{{ trans('employee.employees') }}</span>
                            <span>{{ $countprod[0][15] }}</span>
                        </h1>
                    @endif

                        <span>
                            D : Details - R : Reminder - N : Not Detrmined
                            </span>
                    <!--begin::Table-->
                    <table class="table align-middle table-rounded table-striped table-row-dashed fs-6" id="kt_datatable_table">
                        ...
                        <thead class="bg-light-dark pe-3">
                            ...
                            <tr class="text-start text-dark fw-bold fs-4 text-uppercase gs-0">
                                ...
                                <th class="min-w-125px text-center">{{trans('lang.first_call')}}</th>
                                <th class="min-w-125px text-center">{{trans('lang.type_type')}} {{trans('lang.first_call')}}</th>
                                <th class="min-w-125px text-center">{{trans('lang.second_call')}}</th>
                                <th class="min-w-125px text-center">{{trans('lang.type_type')}} {{trans('lang.second_call')}}</th>
                                <th class="min-w-125px text-center">{{trans('lang.third_call')}}</th>
                                <th class="min-w-125px text-center">{{trans('lang.type_type')}} {{trans('lang.third_call')}}</th>
                        ...
                        @if(isset($countprod))
                                @foreach ( $countprod as $prod)
                                    <tr>
                                        <td class="text-center"></td>
                                        <td class="text-center">{{$prod[0]}}</td>
                                        <td class="text-center">{{$prod[1]}}</td>
                                        <td class="text-center">
                                            <span class="text-info">D({{$prod[2]}})</span><br>
                                            <span class="text-primary">R({{$prod[3]}})</span><br>
                                            <span class="text-danger">N({{$prod[4]}})</span>
                                        </td>
                                        <td class="text-center">{{$prod[5]}}</td>
                                        <td class="text-center">
                                            <span class="text-info">D({{$prod[6]}})</span><br>
                                            <span class="text-primary">R({{$prod[7]}})</span><br>
                                            <span class="text-danger">N({{$prod[8]}})</span>
                                        </td>
                                        <td class="text-center">{{$prod[9]}}</td>
                                        <td class="text-center">
                                            <span class="text-info">D({{$prod[10]}})</span><br>
                                            <span class="text-primary">R({{$prod[11]}})</span><br>
                                            <span class="text-danger">N({{$prod[12]}})</span>
                                        </td>
                                    </tr>
                                @endforeach
                            @endif
...
<script>
  $('#searchbtn').click(function(){
            ...
            const empvisit_id = $('#empvisit_id option:selected').val() || 'all';

            window.location = '{{route("admin.visits.reportprodlist")}}' + '/'  + from_time+ '/' + to_date + '/'  + empvisit_id;
        });
</script>
